...
transition number attributes, so for example it can't work with color strings such as 
"steelblue" and "red". However, d3 being as smart as it is, it can understand when a 
string contains numbers, and smoothly transition the number parts of the string. 
That's why you see a rainbow transition in the example at the top of this article.  

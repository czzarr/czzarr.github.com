...
has the same size as the selection. In our example we use `d3.data()` twice:  
    ...
    // #1
    var rects = chart.selectAll('rect')
                     .data([1 ,4, 5, 6, 24, 8, 12, 1, 1, 20])    // The update will be empty as the selection
                     .enter().doSomething();                     // is empty, the enter will contain all 10
                                                                 // elements

    // #2
    rects.data(newData)     // At this point, rects contains the 10 rectangles, and newData contains
         .doSomething();    // 10 elements, so the update contains all elements, the enter and the exit are empty
...
_value_ can be a function of data and element index, as in our example:  
    ...
    var rects = chart.selectAll('rect').data([1 ,4, 5, 6, 24, 8, 12, 1, 1, 20])
                     ...
                     .attr("x", 0)                                     // All rectangles are 0px from the left of
                                                                       // the container
                                                                       
                     .attr("y", function(d, i) { return 25 * i; } )    // First rectangle (i = 0) is 0px from the
                                                                       // top of the container, second onf (i = 1)
                                                                       // is 25px from the top and so on
                     
                     .attr("width", function(d) { return 20 * d; } )   // Rectangles' width is 20 times the bound data
...
of the bars to animate the graph:  
    ...
    // Simply adding using d3.transition() between new data binding and attribute modification
    // and the change becomes graceful
    rects.data(newData)

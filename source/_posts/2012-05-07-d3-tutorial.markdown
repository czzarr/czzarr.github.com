...
object containing these nodes on which you can apply any d3 function. So in our example:  
    ...
    var chart = d3.select("#d3TutoGraphContainer");   // Select the element with id 'd3TutoGraphContainer'
...
will take place in a container div (here it is 'd3TutoGraphContainer'). In our example:  

    var rects = chart.selectAll("rect");   // Select all SVG rectangles children of this div
...
of each element of _selection_. In our example:  
    ...
    var chart = d3.select("#d3TutoGraphContainer").append("svg");  // Appends a <svg></svg> element inside
                                                                   // the d3TutoGraphContainer div (selection
                                                                   // has only one element)
    
    var rects = chart.selectAll('rect')
                     .data([1 ,4, 5, 6, 24, 8, 12, 1, 1, 20])
                     .enter().append('rect')                    // Inserts on rectangle for each piece of data
...
Of course, at least one of the *enter* and the *exit* has to be empty, both are if we the data 
has the same size as the selection. In our example:  


called the _update_ selection, 
which . 
At this point, the display doesn't change, only the data is affected. In our example:  

    var rects = chart.selectAll('rect')
                     .data([1 ,4, 5, 6, 24, 8, 12, 1, 1, 20]);   // If 'chart' already contained 10 rectangles
                                                                 // this data would be bound to them: 1 to the first
                                                                 // 4 to the second and so on.


This is simple if the data that was bound had the same size as the selection 
since we have a one-to-one mapping. That's often not the case, for example in our tutorial 
the data has 10 elements and the selection zero.  

### The new data size is bigger than the selection size: d3.enter()
...
###  The new data size is smaller than the selection size: d3.exit()
